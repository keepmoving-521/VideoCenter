...
```http
GET /api/v1/media-directories/{directory_id}
```
...
也不能直接停用；应先把另一个目录设为默认。

### 删除目录

```http
DELETE /api/v1/media-directories/{directory_id}
```

删除仅移除目录配置，不删除磁盘目录或其中的媒体文件。删除默认目录时，系统优先选择
剩余的启用目录作为新默认目录；没有启用目录时选择最早创建的剩余目录。删除最后一个
目录后允许暂时不存在默认目录。
...
- 名称和规范化绝对路径分别保持唯一。
- 全库最多只有一个默认目录。
- 每个目录可以独立设置启用状态和自动扫描开关。
- 默认目录在列表中排在最前面，其余目录按 ID 排序。
