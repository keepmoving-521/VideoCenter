from pathlib import Path
from uuid import uuid4

from alembic import command
from alembic.config import Config
from sqlalchemy import create_engine, inspect

from videocenter.core.config import get_settings

PREVIOUS_REVISION = "9d4c7a21b6e0"
BACKGROUND_TASK_REVISION = "73e2b8f940a1"


def test_background_task_migration_creates_and_removes_table(monkeypatch):
    project_root = Path(__file__).resolve().parents[1]
    database_path = project_root / "data" / f"background-task-{uuid4().hex}.db"
    database_url = f"sqlite:///{database_path.as_posix()}"
    config = Config(project_root / "alembic.ini")
    monkeypatch.setenv("VIDEOCENTER_DATABASE_URL", database_url)
    get_settings.cache_clear()

    try:
        command.upgrade(config, BACKGROUND_TASK_REVISION)
        engine = create_engine(database_url)
        inspector = inspect(engine)
        assert "background_tasks" in inspector.get_table_names()
        columns = {column["name"] for column in inspector.get_columns("background_tasks")}
        assert {
            "task_type",
            "status",
            "source_task_id",
            "parent_task_id",
            "progress",
            "processed_items",
            "total_items",
            "attempt",
            "max_attempts",
            "task_payload",
            "task_result",
            "heartbeat_at",
        } <= columns
        engine.dispose()

        command.downgrade(config, PREVIOUS_REVISION)
        engine = create_engine(database_url)
        assert "background_tasks" not in inspect(engine).get_table_names()
        engine.dispose()
    finally:
        get_settings.cache_clear()
        database_path.unlink(missing_ok=True)
